...
class Trilat:
   ...
   # valid if within threshold% from third beacon
   def trilaterate(self,anchors,threshold=10,isVerbose=False):
      x1 = anchors[0][0]
      x2 = anchors[1][0]
      y1 = anchors[0][1]
      y2 = anchors[1][1]
   ...
   def getPointCoord(self,distances,antennas, isVerbose=False):
      beacons = np.zeros((len(distances),3))
      for i in range(len(distances)):
         beacons[i,0] = antennas.iloc[i].x
         beacons[i,1] = antennas.iloc[i].y
         beacons[i,2] = distances[i]
         if beacons[i,0] is None:
            print("None")
      ...
      threshold=5  # point is within treshold% of the distance from the 3rd beacon
      point = self.trilaterate(beacons,threshold,isVerbose)
      if isVerbose:
